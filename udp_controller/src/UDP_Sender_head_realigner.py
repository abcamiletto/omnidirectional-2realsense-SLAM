...
def Callback(data):
    ...
    head_vel = np.array((data.linear.x, data.linear.y), np.float32)	# !! put "data" type received by receiver & check axis (
    motor_x_enable = True
    motor_y_enable = True
...
rospy.Subscriber("cmd_vel_joy", Twist, Callback)	# !! put the topic published by UDP receiver
# ros set rate
f = 100.0
